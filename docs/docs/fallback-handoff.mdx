...
For example, as the final action of Two-Stage-Fallback, you could ask the user, 
"Would you like to be transferred to a human assistant?" and if they say yes, the
bot could send a message in a specific format or with a specific payload like
e.g. "handoff_to_human". When the channel sees this message, it stops listening
...
or if you want to create a `human_handoff` intent.
...
from real conversations. This is especially important for `out_of_scope`
since false positives for this intent can lead to a bad user experience.
...
You can write rules for what should happen for 
...
you can write any rule you'd like for each out-of-scope scenario.
