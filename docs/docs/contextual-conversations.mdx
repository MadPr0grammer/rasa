...
later in the conversation, for example if the user asks about training data formats:

```
stories:
  - story: New user asks about training data formats
    steps:
    - intent: data_format
    - slot_was_set:
      - new_user
    - action: utter_format_overview

  - story: Migrating user asks about training data formats
    steps:
    - intent: data_format
    - action: utter_welcome_migrating_user
    - action: utter_ask_tool
```

### Real Conversations as Stories

### Using ML to generalise

Aside from the more rule-based policies described above, Core also has some ML
